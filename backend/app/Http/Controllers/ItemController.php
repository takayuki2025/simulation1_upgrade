...
class ItemController extends Controller
{
    ...
    {
        return response()->json($useCase->detail($itemId));
    }

    public function sellForm()
    {
        return response()->json(['form' => true], 200);
    }

    public function uploadItemImage(ItemImageRequest $req, ItemUseCase $useCase)
    {
        return response()->json($useCase->uploadImage($req));
    }

    public function create(ExhibitionRequest $req, ItemUseCase $useCase)
    {
        return response()->json($useCase->create($req));
    }

    public function purchaseConfirm($itemId, ItemUseCase $useCase)
    {
        return response()->json($useCase->purchaseConfirm($itemId));
    }
}
...
// class ItemController extends Controller
// {
//     public function index(Request $req, ItemUseCase $useCase)
//     {
//         return response()->json(
//             $useCase->search($req->query('search'))
//         );
//     }
...
//     //     ]);
//     // }
//     public function show(int $id, ItemUseCase $useCase)
//     {
//         return response()->json(
...
//     public function profile_update(Request $r, ProfileUseCase $usecase)
//     {
//         $updated = $usecase->updateProfile($r->user()->id, $r->only([
//             'name', 'post_number', 'address', 'building'
//         ]));
...
//     public function favoriteToggle(Request $req, int $itemId, FavoriteUseCase $useCase)
//     {
//         $result = $useCase->toggle($req->user()->id, $itemId);
...
//     }
//     // 一覧 API（マイページ用）：追加
//     public function favoriteList(Request $req, FavoriteUseCase $useCase)
//     {
//         return response()->json(
//             $useCase->listByUser($req->user()->id)
//         );
//     }
//     // いいね・コメント機能関係
